...
int init_simple_filtergraph(InputStream *ist, OutputStream *ost)
{
    ...
    fg->inputs[0]->frame_queue = av_fifo_alloc(8 * sizeof(AVFrame *)); // 分配8帧
                                         ...
                                         AVFilterInOut *out)
{
    ...
    int ret;
    char name[255];
    // 设置输出，以out_fileindex_outstreamindex进行命名
...
{
    OutputStream *ost = ofilter->ost;               // 找到对应的输出流
    OutputFile *of = output_files[ost->file_index]; //找到对应的输出文件
...
// video filter的输入buffer，和audio的abuffer区分开来
...
// audio filter的输入abuffer
static int configure_input_audio_filter(FilterGraph *fg, InputFilter *ifilter,
                                        AVFilterInOut *in)
{
    AVFilterContext *last_filter;
    const AVFilter *abuffer_filt = avfilter_get_by_name("abuffer"); // audio一定是从abuffer进去
    InputStream *ist = ifilter->ist;
    InputFile *f = input_files[ist->file_index];
    AVBPrint args;
    char name[255];
...
{
    // inputs/outputs 存储filtergraph的filter graph的linked-list, 释放不影响filtergraph
    // 主要用来让外部知道内部目前的存储情况
    AVFilterInOut *inputs, *outputs, *cur;
    int ret, i, simple = filtergraph_is_simple(fg);
    const char *graph_desc = simple ? fg->outputs[0]->ost->avfilter : fg->graph_desc;
    // audio没有filter时为anull， video没有filter时用null
    cleanup_filtergraph(fg);
    if (!(fg->graph = avfilter_graph_alloc())) // 创建avfilter_graph，是一定存在的
        return AVERROR(ENOMEM);

    if (simple)
    {
        OutputStream *ost = fg->outputs[0]->ost; // 对应的输出流？为什么是outputs[0]->ost
        char args[512];
        AVDictionaryEntry *e = NULL;
        ...
        fg->graph->scale_sws_opts = av_strdup(args); // 视频画面使用
        ...
        if (strlen(args))
            args[strlen(args) - 1] = 0;
        av_opt_set(fg->graph, "aresample_swr_opts", args, 0); // 音频重采样使用
    ...
    //解析并创建filter, ffmpeg.c的graph_desc只是中间过程的描述
    ...
    avfilter_inout_free(&inputs);
    // 有没有多个输出filter，将outputs重新绑定？
    for (cur = outputs, i = 0; cur; cur = cur->next, i++)
        configure_output_filter(fg, fg->outputs[i], cur); // 多个输出
    avfilter_inout_free(&outputs);
    // 检查和配置graph所有links和formats
    ...
    for (i = 0; i < fg->nb_outputs; i++)
    {
        OutputFilter *ofilter = fg->outputs[i]; //对应到每个输出
    ...
    for (i = 0; i < fg->nb_outputs; i++)
    {
        ...
        // av_buffersink_set_frame_size 对于audio非常重要，可以每次获取指定的编码器需要的frame_size
    ...
    for (i = 0; i < fg->nb_inputs; i++)
    {
        while (av_fifo_size(fg->inputs[i]->frame_queue))
        {
            ...
            ret = av_buffersrc_add_frame(fg->inputs[i]->filter, tmp); // fifo有数据赶紧放入到filter
            av_frame_free(&tmp);
            if (ret < 0)
                goto fail;
        }
