...
// 每一路输入或输出都对应一个OptionsContext, 每一路输入输出也对应一个OptionGroup
typedef struct OptionsContext
{
    ...
    int video_disable; // 是否禁止视频
    int audio_disable; // 是否禁止音频
...
{
    AVFilterContext *filter;   // 对应一个filter, 具体是什么filter
    struct InputStream *ist;   // 每个input filter对应一个输入流
    struct FilterGraph *graph; // 一个input filter只属于一个graph
    uint8_t *name;
    enum AVMediaType type; // AVMEDIA_TYPE_SUBTITLE for sub2video

    AVFifoBuffer *frame_queue; // 未初始化好 filtergraph时先将frame缓存
...
{
    AVFilterContext *filter; // 对应一个输出filter, 具体是什么filter
...
{
    // 流 到 文件的对应
    int file_index; // stream对应的文件索引号, 如0则为input_files[0].
...
typedef struct InputFile
{
    ...
    // 文件 到 流的对应
    int ist_index;  // 对应 input_streams的位置, 且每个输入文件的stream是紧靠排列
    int nb_streams; // 被放在 input_streams中的流的总个数
...
{
    int file_index;   // 在所属输出文件中的流序号
    int index;        // 所在的输出文件在 output_files 中的序号
    int source_index; // 对应的输入流在 input_streams 中的序号
...
extern InputStream **input_streams; // 输入流的数组
extern int nb_input_streams;        // 数组大小

extern InputFile **input_files; // 输入文件(也可能是设备)的数组
extern int nb_input_files;      // 数组大小

extern OutputStream **output_streams; // 输出流的数组
extern int nb_output_streams;         // 数组大小

extern OutputFile **output_files; // 输出文件的数组
extern int nb_output_files;       // 数组大小
...
// 定义了所有的参数, 参数类型, 参数是否有值, 是放到全局变量中还是放到OptionsContext结构的某个变量中, 或者是调用一个函数来处理参数的值.
